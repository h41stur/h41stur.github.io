...
Para exemplificar, criemos o mesmo programa em C e em Assembly, depois transformá-los em *shellcode*.

Abaixo um programa em C que invoca um *shell:*:
...
![](/img/posts/Pasted%20image%2020240718201118.png)

Quando o transformamos em *shellcode*, vemos que seu tamanho é de 448 Bytes:

![](/img/posts/Pasted%20image%2020240718201259.png)
...
![](/img/posts/Pasted%20image%2020240718201536.png)

Quando o transformamos em *shellcode*, vemos que seu tamanho é de 33 Bytes:

![](/img/posts/Pasted%20image%2020240718201709.png)
...
Para fins de teste dos *shellcodes* é interessante os injetarmos em um programa que irá executá-los, abaixo temos um para arquitetura 32-bits e um para 64-bits:
...
Perceba também, que logo no início, utilizamos o operador `xor` para zerar alguns registradores, garantindo que nenhum `NULL Byte` entre no código.
...
A *string* de saída pode ser inserida no script de teste:
...
Carrego o endereço de "/bin/sh" na memória como uma *string* e, em seguida, passo esse endereço para a função. Quando os ponteiros são desreferenciados, a memória de destino conterá a string "/bin/sh".
...
E após a compilação, temos o *shellcode* em operação:
...
No Arch Linux, podemos, por exemplo, consultar o número da *syscall execve*  na arquitetura 64-bits da seguinte forma:
...
Assim como fizemos anteriormente, criaremos um simples programa para injetar o *shellcode* e testá-lo no ambiente.
...
A função [WinExec()](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-winexec) na `kernell32` pode ser usada para iniciar qualquer programa que o usuário executando o processo tenha permissão.
...
mov  eax, esp         ; salvando o pontairo para a string "calc.exe" em ebx
...
Um fato importante para saber, é que a maioria das funções do Windows, são acessadas através das `DLL Primárias`: **ntdll.dll**, **ernel32.dll** e **kernelbase.dll**. Estas são DLL de extrema importância para o funcionamento do kernel, portanto, toda vez que executamos um binário, elas são automaticamente carregadas.
...
int main() {
    ...
    // Chama a calculadora do Windows usando WinExec
...
Vamos carregar este programa no `x32dbg` e "debugar" seu comportamento.
...
Se procurarmos pela função `WinExec` utilizada no programa, veremos que ela é carregada pela biblioteca `kernel32.dll` com o endereço `0x76705140`. Podemos confirmar pesquisando pela função em nosso programa `getarrd.exe`:
...
Dos passos 3 ao 5 já fizemos algo idêntico no exemplo anterior da caixa de mensagem, mas como proceder com os passos 1 e 2?
...
O *offset*  para `_PEB_LDR_DATA` é `0x00c`. Também podemos utilizar o mesmo `dt nt!_PEB_LDR_DATA` comando para verificar o que existe na estrutura `_PEB_LDR_DATA`.
...
Basicamente, o cálculo dos endereços das tabelas segua a lógica de que: endereço da tabela = endereço da tabela anterior + tamanho da tabela anterior + 4 (tamanho do endereço 4 bytes), ou seja:
...
Seguindo o fluxo para invocar a função, vimos que a partir do passo **6** não temos uma forma automatizada de encontrar, tivemos a facilidade de procurar manualmente, mas se uma atualização alterar o ordinal da função, o *shellcode* deixa de funcionar. Portanto, vamos escrever um programa, que fará um *loop* na `EXPORT Name Pointer Table` a procura da *string*  que contenha o nome da função desejada passando por cada ordinal de forma incremental.

Primeiramente, precisamos enviar o nome da função a ser procurada na *stack*, ela precisa estar em hexadecimal e em ordem inversa separada a cada 8 bytes. Usaremos o programa em Go que utilizamos anteriormente.
...
Este trecho abaixo, fará a procura do endereço da função utilizando os 9 passos listados acima, muita atenção, pois é a continuação do programa que procura o endereço base da kernel32.dll, portanto ele conta que este endereço base já esteja armazenado em `eax`.
...
E é exatamente por este motivo que a `EXPORT Ordinal Table` começa em `004` e não em `001`, portanto nosso contador precisa de uma correção.
...
Estes são os passos primordiais para um bom *shellcode* no ambiente Windows com endereços dinâmicos. No exemplo utilizado, foi utilizada a função `WinExec()`, porém existe uma infinidade de funções úteis, como `CreateProcess()`, `GetProcess()`, entre outras. Os passos mais importantes são o de encontrar o endereço base da kernel32.dll, entender a estrutura interna desta DLL e entender como funcionam as estruturas `TEB` e `PEB`.
