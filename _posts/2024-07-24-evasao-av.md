...
- **`hHandle`**: Um manipulador para o objeto de sincronização. Neste caso, `th` é o manipulador da thread criada anteriormente com `CreateThread`.
...
Para dificultar este processo, podemos criptografar nosso *shellcode* que está *hardcoded* no programa e descriptografá-lo em tempo de execução. Neste ponto, existe uma infinidade de técnica e algoritmos que podemos usar, quanto melhor a eficácia da criptografia, mais difícil será a identificação do *shellcode*.
...
> É por isso que a operação `xor eax, eax` zera o registrador `EAX`, uma vez que o XOR é executado em bits iguais, o resultado sempre será 0.
...
A partir desta saída, podemos criar o *shellcode* que será armazenado no programa, mas faremos uma alteração. No programa atual, declaramos a variável `unsigned char shellcode[]` que é um *array*, porém inserimos o *shellcode* como uma única *string*. Desta vez, criaremos de fato um *array* de *bytes*.
...
Se formos na aba [*detrails*](https://www.virustotal.com/gui/file/008b76ba3c5f278c5e021c52d2d78b8d0feea242b711017c65a53ae0537a66af/details) da análise feita pelo VirusTotal na sessão de ***Imports***, podemos expandir a kernel32.dll e verificar as funções importadas pelo programa.
...
Tomando como exemplo a função `VirtualAlloc`, ao consultar o [MSDN](https://learn.microsoft.com/en-us/windows/win32/api/memoryapi/nf-memoryapi-virtualalloc), vemos que sua sintaxe é:
...
Então podemos criar uma variável global do tipo VirtualAlloc que será um ponteiro para a VirtualAlloc original.

```cpp
LPVOID (WINAPI * pVA)(LPVOID lpAddress, SIZE_T dwSize, DWORD flAllocationType, DWORD  flProtect);
```

Agora podemos capturar seu endereço com a função `GetProcAddress` e mudar a chamada para `pVA`.
...
Esta implementação é a mais simples e pode não ser efetiva em muitos antivírus, porém é uma técnica bastante utilizada que vale a pena utilizar.
...
Neste caso, uma das técnicas mais utilizadas é justamente alocar quma grande quantidade de memória dinâmica antes de se iniciar a lógica da exploração. Por exemplo, podemos alocar na *heap* `500MB` de memória e preenchê-la com zeros:
